#!/usr/bin/env python3
"""
Live-Test des XML-Scrapers
Testet den Scraper mit echten Daten von theyworkforyou.com
"""
...
def test_scraper_live():
    """Testet den Scraper mit echten Daten"""
    print("🚀 Live-Test des XML-Scrapers")
    print("=" * 50)
    ...
    test_config = {
        "start_year": 2020,
        "end_year": 2021,
        "max_workers": 2,  # Reduziert für Test
        "delay_between_requests": 2.0,  # Höhere Verzögerung für Höflichkeit
    ...
    try:
        ...
        # 3. Teste Download von wenigen Dateien
        if links:
            print(f"\n3. Teste Download von {min(3, len(links))} Dateien...")
            
            test_links = links[:3]  # Nur erste 3 Dateien für Test
...
def main():
    """Hauptfunktion"""
    print("🧪 Parliament Data Processing System - Live Test")
    print("=" * 60)
    print(f"Startzeit: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print()
    
    # Warnung für echte Website
    print("⚠️  WICHTIG: Dieser Test lädt echte Daten von theyworkforyou.com herunter!")
    print("   Bitte respektieren Sie die Server-Ressourcen und robots.txt")
    print()
    
    # Benutzer-Bestätigung
    response = input("Möchten Sie den Live-Test fortsetzen? (y/N): ")
    if response.lower() != 'y':
    ...
    # Führe Tests durch
    scraper_success = test_scraper_live()
