"""
Skript zum Ausführen der Brexit-Speech-Klassifikation
Verwendet Google Gemini API zur Klassifikation aller Reden
"""
...
def main():
    """Hauptfunktion für Brexit-Speech-Klassifikation"""
    
    parser = argparse.ArgumentParser(description='Brexit-Speech-Klassifikation mit Google Gemini')
    ...
    parser.add_argument('--output-db', 
                       default='database/data/brexit_speeches.db',
                       help='Pfad zur Ausgabe-Datenbank')
    parser.add_argument('--batch-size', 
                       type=int, 
                       default=50,
                       help='Batch-Größe für Verarbeitung')
    parser.add_argument('--max-speeches', 
                       type=int, 
                       default=None,
                       help='Maximale Anzahl zu verarbeitender Reden (für Tests)')
    parser.add_argument('--api-key', 
                       help='Google API Key (falls nicht in Umgebungsvariablen)')
    parser.add_argument('--test', 
                       action='store_true',
                       help='Test-Modus: verarbeite nur 100 Reden')
    
    args = parser.parse_args()
    
    # Test-Modus
    if args.test:
        args.max_speeches = 100
        print("🧪 TEST-MODUS: Verarbeite nur 100 Reden")
    ...
    print("🚀 Brexit-Speech-Klassifikation gestartet")
    ...
    if args.max_speeches:
        print(f"🔢 Max. Reden: {args.max_speeches}")
    print()
    ...
    try:
        ...
        # Zeige Ergebnisse
        print("\n" + "="*60)
        print("📊 KLASSIFIKATIONS-ERGEBNISSE")
        print("="*60)
